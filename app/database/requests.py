...
async def default_plans():
    async with async_session() as session:
        session.add(Plan(name="Исследователь (Бесплатно)", duration = 1, price=0, description=description_1))
        session.add(Plan(name="Безопасная Троица", duration = 3, price=2, description=description_2))
        session.add(Plan(name="Мастер Обфускации", duration = 6, price=4, description=description_3))
        session.add(Plan(name="Элитный Страж", duration = 12, price=6, description=description_4))
